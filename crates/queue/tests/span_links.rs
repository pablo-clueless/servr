...
fn exported_spans(job: Job) -> Vec<SpanData> {
...
#[test]
fn the_execution_span_links_to_the_enqueue_span() {
    let spans = exported_spans(delayed_job_with_trace());
...
#[test]
fn the_execution_span_does_not_descend_from_the_enqueue_span() {
    let spans = exported_spans(delayed_job_with_trace());
    let execution = execution_span(&spans);
...
#[test]
fn a_job_enqueued_without_trace_context_still_runs() {
    let job = Job::new(RunId::new(), "noop", chrono::Utc::now());
    let spans = exported_spans(job);
...
#[test]
fn the_execution_span_records_its_outcome() {
    let spans = exported_spans(delayed_job_with_trace());
    let execution = execution_span(&spans);
